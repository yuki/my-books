...
A la hora de crear un registro, Laravel por defecto hace uso de la ruta “create”, por lo que deberemos crear un botón en la vista principal que nos mande a la URL “/posts/create”, por lo que la vista de creación será \configfile{create.blade.php}.
...
\begin{itemize}
    ...
    \item \inlineconsole{@csrf}: es un atributo especial de Blade que genera un atributo oculto “input” en el formulario donde se guarda el token \href{https://es.wikipedia.org/wiki/Cross-site_request_forgery}{CSRF}, para posteriormente comprobar que es correcto.
\end{itemize}
...
También hay que tener en cuenta que debido a cómo funciona el protocolo HTTP con los formularios, \href{https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/PUT}{el método PUT no se puede utilizar en formularios diréctamente}, por lo que debemos añadir \inlineconsole{@method('PUT')} para que lo genere \href{https://laravel.com/docs/10.x/blade#method-field}{oculto} en el formulario.

\exercisebox{Dado que el formulario de crear y actualizar es prácticamente igual, sería interesante crear una única vista que sirva para ambos métodos.}
...
\begin{mycode}{Vista del index.blade.php}{html+smarty}{{\footnotesize }}
...
Y por último el controlador debe borrar el objeto cuando se llama a la función \textbf{delete}:
